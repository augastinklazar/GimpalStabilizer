...
void calculate_angles() {
  ...
  angle_x = (gyro_x * GYRO_SCALE_FACTOR) * (float)millis() / 1000.0f;
  angle_y = (gyro_y * GYRO_SCALE_FACTOR) * (float)millis() / 1000.0f;
...
void calculate_integrals() {
  ...
  integral_x += error_x * (float)millis() / 1000.0f;
  integral_y += error_y * (float)millis() / 1000.0f;
}

void calculate_derivatives() {
  // Calculate the derivative readings from the error readings
  derivative_x = (error_x - previous_error_x) / (float)millis() / 1000.0f;
  derivative_y = (error_y - previous_error_y) / (float)millis() / 1000.0f;
}
...
void update_servos() {
  ...
  analogWrite(SERVO_X_PIN, output_x);
  analogWrite(SERVO_Y_PIN, output_y);
}
...
void loop() {
  ...
  // Print the angle readings to the serial monitor
